...
def write_repo_files() -> None:
    ...
    (pkg_dir / "__init__.py").write_text(
        '__all__ = ["__version__", "UltraAIEngine", "AIRecommendation", "EvidenceSink"]\n'
        '__version__ = "0.1.0"\n\n'
        "from .engine import UltraAIEngine, AIRecommendation, EvidenceSink\n",
        encoding="utf-8",
    )

    # engine.py = header portion of this generator (no write_repo_files)
    header = (REPO_ROOT / "generators" / "gen_phase10a_ultraai.py").read_text(encoding="utf-8").split("def write_repo_files()", 1)[0]
    (pkg_dir / "engine.py").write_text(header + "\n", encoding="utf-8")

    tests_dir = REPO_ROOT / "tests"
    tests_dir.mkdir(parents=True, exist_ok=True)

    (tests_dir / "test_phase10a_ultraai_policy_bound.py").write_text(
...
if __name__ == "__main__":
    write_repo_files()
    print("Phase 10A UltraAI files written.")
